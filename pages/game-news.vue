<template>
  <main class="container">
    <SearchBar :search="search" :sort="sort" :sort-list="sortList" @search-click="searchGameNews" />
    <CardLayout
    ...
    >
      <Card
        ...
        class="card"
        :title="game.headline"
        :image="'https://terrigen-cdn-dev.marvel.com/content/prod/1x/' + game.image.filename"
        :click="openDetail"
        :id="game.link.link"
      />
    </CardLayout>
...
export default {
  ...
  methods: {
    ...
    nextPage() {
      this.page++;
      this.getGameNews().then((result) => {
        this.gameNews = [...this.gameNews, ...result];
        setTimeout(() => {
          window.scrollBy({
            top: 500,
            behavior: "smooth",
    ...
    async openDetail(id) {
      ...
      if (item) {
        window
          .open("https://www.marvel.com" + item.link.link, "_blank")
          .focus();
      }
    },
  },
